...
class MatrixVectorMultiplyFunctor
{
  ...
  BArrayType* BArray;
  XArrayType* XArray;
  vtkVortexCoreTest* Filter;

public:
  MatrixVectorMultiplyFunctor(
    AArrayType* aArray, BArrayType* bArray, XArrayType* xArray, vtkVortexCoreTest* filter)
    : AArray(aArray)
    , BArray(bArray)
    , XArray(xArray)
    , Filter(filter)
...
{
  template <typename AArrayType, typename BArrayType, typename XArrayType>
  void operator()(AArrayType* aArray, BArrayType* bArray, XArrayType* xArray, vtkVortexCoreTest* filter)
  {
...
// vtkGradientFilter layout for a 9-component Jacobian J_ij (row-major i*3+j):
// output index (i*3+j)*3+k is d(J_ij)/d(x_k), k=0,1,2 -> 27 components per point.
// Higher-order parallel-vector line uses jerk b_i = (J * J * v)_i + sum_j sum_k (d J_ij / d x_k) * v_j * v_k.
class JacobianGradientContractFunctor
{
  vtkDataArray* JacobianPrime;
...
public:
  JacobianGradientContractFunctor(vtkDataArray* jp, vtkDataArray* vel, vtkDataArray* jacobian, vtkDoubleArray* jerk,
    vtkVortexCoreTest* filter)
    : JacobianPrime(jp)
    , Velocity(vel)
    , Jacobian(jacobian)
  ...
  void operator()(vtkIdType begin, vtkIdType end)
  {
    ...
    for (vtkIdType pt = begin; pt < end; ++pt)
    {
      if (isFirst)
      {
        this->Filter->CheckAbort();
      ...
      this->JacobianPrime->GetTuple(pt, g);
      this->Velocity->GetTuple(pt, v);
      this->Jacobian->GetTuple(pt, J);
      
      double out[3] = { 0., 0., 0. };
      
      for (int i = 0; i < 3; ++i)
      {
        // 1. Calculate J * (J * v) term
        double a_i = 0.0;
        for (int j = 0; j < 3; ++j) {
          double J_v_j = 0.0;
          for (int k = 0; k < 3; ++k) {
        ...
        // 2. Calculate T * v * v term
        for (int j = 0; j < 3; ++j)
        {
          for (int k = 0; k < 3; ++k)
          {
            out[i] += g[(i * 3 + j) * 3 + k] * v[j] * v[k];
          }
        }
      }
      this->Jerk->SetTuple(pt, out);
    }
  }
};
...
public:
  ComputeCriteriaFunctor(JacobianArrayType* jacobianArray,
    AcceptedPointsArrayType* acceptedPointsArray, vtkVortexCoreTest* filter)
...
class vtkParallelVectorsForVortexCoreTest : public vtkParallelVectors
{
public:
  static vtkParallelVectorsForVortexCoreTest* New();
  vtkTypeMacro(vtkParallelVectorsForVortexCoreTest, vtkParallelVectors);
...
protected:
  vtkParallelVectorsForVortexCoreTest() = default;
  ~vtkParallelVectorsForVortexCoreTest() override = default;
...
private:
  vtkParallelVectorsForVortexCoreTest(const vtkParallelVectorsForVortexCoreTest&) = delete;
  void operator=(const vtkParallelVectorsForVortexCoreTest&) = delete;
};
vtkStandardNewMacro(vtkParallelVectorsForVortexCoreTest);

//------------------------------------------------------------------------------
void vtkParallelVectorsForVortexCoreTest::Prefilter(
...
//------------------------------------------------------------------------------
bool vtkParallelVectorsForVortexCoreTest::AcceptSurfaceTriangle(
...
//------------------------------------------------------------------------------
bool vtkParallelVectorsForVortexCoreTest::ComputeAdditionalCriteria(
...
//------------------------------------------------------------------------------
vtkStandardNewMacro(vtkVortexCoreTest);

//------------------------------------------------------------------------------
vtkVortexCoreTest::vtkVortexCoreTest()
...
//------------------------------------------------------------------------------
vtkVortexCoreTest::~vtkVortexCoreTest() = default;

//------------------------------------------------------------------------------
int vtkVortexCoreTest::RequestData(
  vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (this->HigherOrderMethod)
  {
    ...
    vtkDataArray* jacobianPrime = vtkDataArray::SafeDownCast(
      gradientPrime->GetOutput()->GetPointData()->GetAbstractArray("jacobian_prime"));
    if (jacobianPrime == nullptr)
    {
      vtkErrorMacro(<< "Could not access jacobian_prime array from vtkGradientFilter");
      return 0;
    }
    if (jacobianPrime->GetNumberOfComponents() != 27)
    {
      vtkErrorMacro(<< "Expected jacobian_prime with 27 components (gradient of 9-component "
                       "Jacobian), got "
                    << jacobianPrime->GetNumberOfComponents());
      return 0;
    }

    vtkNew<vtkDoubleArray> jerk;
    jerk->SetName("jerk");
    jerk->SetNumberOfComponents(3);
    jerk->SetNumberOfTuples(velocity->GetNumberOfTuples());

    // Updated to pass the Jacobian array into the functor
    JacobianGradientContractFunctor jerkFunctor(jacobianPrime, velocity, jacobian.Get(), jerk, this);
    vtkSMPTools::For(0, velocity->GetNumberOfTuples(), jerkFunctor);
  ...
  // fields are parallel.
  vtkNew<vtkParallelVectorsForVortexCoreTest> parallelVectorsForVortexCore;
...
//------------------------------------------------------------------------------
int vtkVortexCoreTest::FillInputPortInformation(int, vtkInformation* info)
{
...
//------------------------------------------------------------------------------
void vtkVortexCoreTest::PrintSelf(ostream& os, vtkIndent indent)
{
