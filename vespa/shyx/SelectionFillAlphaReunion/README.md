...
对未选中/选中两部分都补洞；对选中部分做 Alpha Wrap（除非 `SkipAlphaWrapping`）；再 CGAL union。默认 `EnableBridgeCleanup` ON：接缝处可分别勾选局部 isotropic remesh 与 smooth（默认都开；可只 remesh 不 smooth）。清理 mask 总会写成 cell 数组 `SHYXBridgeCleanupMask`（1 = cleanup patch；remesh 后对应该区域）。
