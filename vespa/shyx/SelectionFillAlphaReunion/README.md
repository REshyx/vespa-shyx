...
对未选中/选中两部分都补洞；对选中部分做 Alpha Wrap（除非 `SkipAlphaWrapping`）；再 CGAL union。**Alpha / Offset 始终是绝对长度**；默认 **0** 用选区 AABB 最长边的 **10%**（每次 Apply 按当前选区刷新）。面板 **Reset** 通过自定义 Domain `SHYXSelectionBoundsDomain` 填入同一建议值（选区，不是整网）；**Scale** 只是把框里的数 ×0.5 / ×2。默认 `EnableBridgeCleanup` ON：接缝处可分别勾选局部 isotropic remesh 与 smooth（默认都开；可只 remesh 不 smooth）。清理 mask 总会写成 cell 数组 `SHYXBridgeCleanupMask`（1 = cleanup patch；remesh 后对应该区域）。
