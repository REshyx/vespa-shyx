...
模块 NAME `vtkSHYXSurfaceToVolumeMesh`；C++ / XML class 仍是 `vtkCGALSurfaceToVolumeMesh`。菜单 **Filters → SHYX**（**不在** Vascular 工具条）。
...
| **`CellSize`** | 0.0 | **四面体外接球半径上限**（不是四面体体积）。`0.0` 表示不施加该约束。 |
