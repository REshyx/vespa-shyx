...
| 参数名称 | 类型 | 默认值 | 效果和含义 |
| :--- | :--- | :--- | :--- |
| **`MinEdgeLength`** | `double` | `-1.0` | **最小边长限制**。重网格化后三角形边的最短下限。如果设置为 `<= 0`，系统将自动计算为输入模型包围盒对角线长度的 **0.5%**。 |
| **`MaxEdgeLength`** | `double` | `-1.0` | **最大边长限制**。重网格化后三角形边的最长上限。如果设置为 `<= 0`，将自动计算为模型包围盒对角线长度的 **5%**。在平坦区域，网格边长会尽量逼近此值。 |
...
*(注：系统提供 `RefreshSuggestedEdgeLengthsFromBounds()` 方法以辅助 ParaView UI 界面交互。点击按钮即可基于当前模型尺寸获取推荐的默认最大/最小边长。)*