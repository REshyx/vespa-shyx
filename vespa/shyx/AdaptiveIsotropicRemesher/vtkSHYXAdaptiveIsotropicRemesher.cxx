...
private:
  ...
  void compute_sizes_(FaceGraph& fg)
  {
    ...
    pmp::interpolated_corrected_curvatures(
      fg, pmp::parameters::vertex_principal_curvatures_and_directions_map(curv_map));

    for (auto v : vertices(fg))
    {
      const Principal vc = get(curv_map, v);
      const FT max_abs =
        (CGAL::max)(CGAL::abs(vc.max_curvature), CGAL::abs(vc.min_curvature));
