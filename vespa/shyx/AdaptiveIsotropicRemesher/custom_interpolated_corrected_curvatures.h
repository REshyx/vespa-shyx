...
/** Populated by PrepareIccVertexNormalsForAdaptiveSizing when the feature mask is on:
 *  `v:vespa_icc_n_mask`, `v:vespa_icc_n_nonmask`, `f:vespa_icc_in_mask`.
 *  When active, per-**vertex** ICC aggregates sum only **non-mask** incident faces (ball expansion
 *  still traverses mask faces without adding their measures). Principal-direction extraction uses
 *  `vn_nonmask` as the reference normal. */
struct VespaIccDualNormalBundle
{
  bool active = false;
  CGAL_Surface::Property_map<CGAL_Surface::Vertex_index, CGAL_Kernel::Vector_3> vn_mask;
  CGAL_Surface::Property_map<CGAL_Surface::Vertex_index, CGAL_Kernel::Vector_3> vn_nonmask;
  CGAL_Surface::Property_map<CGAL_Surface::Face_index, bool> face_in_mask;
};

inline bool VespaIccTryLoadDualNormalBundle(const CGAL_Surface* host, VespaIccDualNormalBundle& out)
{
  out.active = false;
  if (host == nullptr)
  {
  ...
  const auto om =
    host->property_map<CGAL_Surface::Vertex_index, CGAL_Kernel::Vector_3>("v:vespa_icc_n_mask");
  const auto on =
    host->property_map<CGAL_Surface::Vertex_index, CGAL_Kernel::Vector_3>("v:vespa_icc_n_nonmask");
  const auto of = host->property_map<CGAL_Surface::Face_index, bool>("f:vespa_icc_in_mask");
  if (!om.has_value() || !on.has_value() || !of.has_value())
  {
    return false;
  }
  out.vn_mask      = *om;
...
template <typename Vertex_normal_map, typename face_descriptor, typename vertex_descriptor>
inline CGAL_Kernel::Vector_3 VespaIccCornerNormalForFace(const VespaIccDualNormalBundle* dual,
  Vertex_normal_map vnm, face_descriptor f, vertex_descriptor v)
{
  if (dual == nullptr || !dual->active)
  {
    return get(vnm, v);
  }
  const bool fm = get(dual->face_in_mask, f);
  return get(fm ? dual->vn_mask : dual->vn_nonmask, v);
}

/** True when @a f should not contribute to per-vertex ICC aggregates (dual bundle: mask faces). */
template <typename face_descriptor>
inline bool VespaIccSkipMaskedFaceForVertexAggregate(const VespaIccDualNormalBundle* dual,
  face_descriptor f)
{
  return dual != nullptr && dual->active && get(dual->face_in_mask, f);
}

/** Normal used to constrain principal directions when vertex aggregates are restricted to non-mask faces. */
template <typename GT, typename Vertex_normal_map, typename vertex_descriptor>
inline typename GT::Vector_3 VespaIccVertexNormalForPrincipalSolve(const VespaIccDualNormalBundle* dual,
  Vertex_normal_map vnm,
  vertex_descriptor v)
{
  if (dual != nullptr && dual->active)
  {
    return get(dual->vn_nonmask, v);
  }
  return get(vnm, v);
}
...
struct Custom_principal_curvatures_and_directions {
...
template<typename GT>
Custom_principal_curvatures_and_directions<GT> principal_curvatures_and_directions_from_anisotropic_measures(
...
)
{
  ...
  if (eigensolver.info() != Eigen::Success)
    return Custom_principal_curvatures_and_directions<GT>();
...
Vertex_measures<GT> interpolated_corrected_measures_one_vertex_no_radius(
  ...
  const VPM vpm,
  const VNM vnm,
  const VespaIccDualNormalBundle* vespa_dual_bundle = nullptr
)
{
  ...
  for (face_descriptor f : faces_around_target(halfedge(v, pmesh), pmesh)) {
    ...
    {
      if (VespaIccSkipMaskedFaceForVertexAggregate(vespa_dual_bundle, f))
      {
        continue;
      }
      ...
      for (vertex_descriptor vi : vertices_around_face(halfedge(f, pmesh), pmesh))
      {
        ...
        const Vector_3 ui = VespaIccCornerNormalForFace(vespa_dual_bundle, vnm, f, vi);
...
)
{
  ...
  while (!bfs_queue.empty()) {
    ...
    {
      const Point_3& pi = get(vpm, vi);
      const Vector_3 ui = VespaIccCornerNormalForFace(vespa_dual_bundle, vnm, fi, vi);
      x.push_back(Vector_3(pi.x(), pi.y(), pi.z()));
      u.push_back(ui);
    }
    ...
    if (!is_zero(f_ratio))
    {
      if (!VespaIccSkipMaskedFaceForVertexAggregate(vespa_dual_bundle, fi))
      {
...
template <class T>
void set_value(const T&, CGAL::internal_np::Param_not_found)
{}
...
{
  typedef typename CGAL::GetGeomTraits<PolygonMesh, NamedParameters>::type GT;
  typedef typename CGAL::GetVertexPointMap<PolygonMesh, NamedParameters>::const_type Vertex_position_map;

  typedef CGAL::dynamic_vertex_property_t<typename GT::Vector_3> Vector_map_tag;
  typedef typename boost::property_map<PolygonMesh, Vector_map_tag>::const_type Default_vector_map;
  typedef typename CGAL::internal_np::Lookup_named_param_def<CGAL::internal_np::vertex_normal_map_t,
    NamedParameters,
    Default_vector_map>::type Vertex_normal_map;

  using CGAL::Polygon_mesh_processing::parameters::choose_parameter;
  using CGAL::Polygon_mesh_processing::parameters::get_parameter;
  using CGAL::Polygon_mesh_processing::parameters::is_default_parameter;

  VespaIccDualNormalBundle dual_storage;
  const VespaIccDualNormalBundle* dual_ptr = nullptr;
  if (vespa_dual_normal_property_host != nullptr &&
    VespaIccTryLoadDualNormalBundle(vespa_dual_normal_property_host, dual_storage))
  {
    dual_ptr = &dual_storage;
  }

  Vertex_position_map vpm = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_point),
    CGAL::get_const_property_map(CGAL::vertex_point, pmesh));

  Vertex_normal_map vnm = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_normal_map),
    get(Vector_map_tag(), pmesh));

  // if the normal map is not provided, compute it
  if (is_default_parameter<NamedParameters, CGAL::internal_np::vertex_normal_map_t>::value)
  ...
  typename GT::FT radius = choose_parameter(get_parameter(np, CGAL::internal_np::ball_radius), -1);
  ...
  const bool is_mean_curvature_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_mean_curvature_t>::value;
  const bool is_Gaussian_curvature_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_Gaussian_curvature_t>::value;
  const bool is_principal_curvatures_and_directions_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_principal_curvatures_and_directions_t>::value;
  ...
  if (is_mean_curvature_selected) {
    set_value(!is_zero(vertex_measures.area_measure) ? 0.5 * vertex_measures.mean_curvature_measure / vertex_measures.area_measure : 0,
              get_parameter(np, CGAL::internal_np::vertex_mean_curvature));
  }

  if (is_Gaussian_curvature_selected) {
    set_value(!is_zero(vertex_measures.area_measure) ? vertex_measures.gaussian_curvature_measure / vertex_measures.area_measure : 0,
              get_parameter(np, CGAL::internal_np::vertex_Gaussian_curvature));
  }

  if (is_principal_curvatures_and_directions_selected) {
    // compute the principal curvatures and directions from the anisotropic measures
    const typename GT::Vector_3 v_normal = VespaIccVertexNormalForPrincipalSolve<GT>(dual_ptr, vnm, v);
    const Custom_principal_curvatures_and_directions<GT> principal_curvatures_and_directions = principal_curvatures_and_directions_from_anisotropic_measures<GT>(
      vertex_measures.anisotropic_measure,
      vertex_measures.area_measure,
      v_normal,
      avg_edge_length
      );
    set_value(principal_curvatures_and_directions, get_parameter(np, CGAL::internal_np::vertex_principal_curvatures_and_directions));
  }
}


template<typename PolygonMesh, class NamedParameters>
class Custom_interpolated_corrected_curvatures_computer
{
  typedef typename CGAL::GetGeomTraits<PolygonMesh, NamedParameters>::type GT;
  ...
  typedef CGAL::dynamic_vertex_property_t<Vector_3> Vector_map_tag;
  typedef typename boost::property_map<PolygonMesh, Vector_map_tag>::const_type Default_vector_map;
  typedef typename CGAL::internal_np::Lookup_named_param_def<CGAL::internal_np::vertex_normal_map_t,
    NamedParameters,
    Default_vector_map>::type Vertex_normal_map;

  typedef CGAL::Constant_property_map<vertex_descriptor, FT> Default_scalar_map;
  typedef typename CGAL::internal_np::Lookup_named_param_def<CGAL::internal_np::vertex_mean_curvature_map_t,
    NamedParameters,
    Default_scalar_map>::type Vertex_mean_curvature_map;

  typedef typename CGAL::internal_np::Lookup_named_param_def<CGAL::internal_np::vertex_Gaussian_curvature_map_t,
    NamedParameters,
    Default_scalar_map>::type Vertex_Gaussian_curvature_map;

  typedef CGAL::Constant_property_map<vertex_descriptor, Custom_principal_curvatures_and_directions<GT>> Default_principal_map;
  typedef typename CGAL::internal_np::Lookup_named_param_def<CGAL::internal_np::vertex_principal_curvatures_and_directions_map_t,
    NamedParameters,
...
private:
  const PolygonMesh& pmesh;
  const CGAL_Surface* vespa_dual_prop_host_;
  VespaIccDualNormalBundle vespa_dual_bundle_;
  ...
  void set_named_params(const NamedParameters& np)
  {
    ...
    vnm = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_normal_map),
      get(Vector_map_tag(), pmesh));
    ...
    // if no radius is given, we pass -1 which will make the expansion be only on the incident faces instead of a ball
    const FT radius = choose_parameter(get_parameter(np, CGAL::internal_np::ball_radius), -1);
    ...
    is_mean_curvature_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_mean_curvature_map_t>::value;
    is_Gaussian_curvature_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_Gaussian_curvature_map_t>::value;
    is_principal_curvatures_and_directions_selected = !is_default_parameter<NamedParameters, CGAL::internal_np::vertex_principal_curvatures_and_directions_map_t>::value;

    mean_curvature_map = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_mean_curvature_map), Default_scalar_map());
    gaussian_curvature_map = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_Gaussian_curvature_map), Default_scalar_map());
    principal_curvatures_and_directions_map = choose_parameter(get_parameter(np, CGAL::internal_np::vertex_principal_curvatures_and_directions_map), Default_principal_map());
  }
...
private:
  ...
  void interpolated_corrected_selected_measures_all_faces()
  {
    ...
    for (face_descriptor f : faces(pmesh))
    {
      for (vertex_descriptor v : vertices_around_face(halfedge(f, pmesh), pmesh))
      {
        ...
        const VespaIccDualNormalBundle* corners_dual =
          vespa_dual_bundle_.active ? &vespa_dual_bundle_ : nullptr;
        u.push_back(VespaIccCornerNormalForFace(corners_dual, vnm, f, v));
      }
  ...
  Vertex_measures<GT> expand_interpolated_corrected_measure_vertex_no_radius(vertex_descriptor v)
  {
    ...
    for (face_descriptor f : faces_around_target(halfedge(v, pmesh), pmesh)) {
      ...
      if (VespaIccSkipMaskedFaceForVertexAggregate(dual_corner, f))
        continue;
  ...
  void compute_selected_curvatures() {
    ...
    for (vertex_descriptor v : vertices(pmesh))
    {
      ...
      if (is_principal_curvatures_and_directions_selected) {
        ...
        const Vector_3 v_normal = VespaIccVertexNormalForPrincipalSolve<GT>(
          vespa_dual_bundle_.active ? &vespa_dual_bundle_ : nullptr, vnm, v);
        const Custom_principal_curvatures_and_directions<GT> principal_curvatures_and_directions = principal_curvatures_and_directions_from_anisotropic_measures<GT>(
          vertex_measures.anisotropic_measure,
          vertex_measures.area_measure,
          v_normal,
...
*     \cgalParamType{a class model of `WritablePropertyMap` with
*                    `boost::graph_traits<PolygonMesh>::%vertex_descriptor`
*                    as key type and `Custom_principal_curvatures_and_directions<GT>` as value type.}
...
*     \cgalParamDefault{`get(CGAL::dynamic_vertex_property_t<GT::Vector_3>(), pmesh)`.}
...
*     \cgalParamDescription{a reference to a `Custom_principal_curvatures_and_directions<GT>` object to store the principal curvatures and directions at the vertex `v`.}
*     \cgalParamType{`std::reference_wrapper<Custom_principal_curvatures_and_directions<GT>>`.}
*     \cgalParamExtra{If this parameter is omitted, principal curvatures and directions will not be computed.}
*  \cgalParamNEnd
*
*   \cgalParamNBegin{ball_radius}
...
template<typename VertexDescriptor,
         ...
         typename CGAL_NP_TEMPLATE_PARAMETERS>
void custom_interpolated_corrected_curvatures(VertexDescriptor v,
                                       const PolygonMesh& pmesh,
                                       const CGAL_NP_CLASS& np = CGAL::Polygon_mesh_processing::parameters::default_values(),
                                       const CGAL_Surface* vespa_dual_normal_property_host = nullptr)
{
  internal::custom_interpolated_corrected_curvatures_one_vertex(
    pmesh, v, np, vespa_dual_normal_property_host);
}
