...
5. **可选端帽后处理 (`AppendCapEndpoints`)**：若输入已带端帽标记（如 Vessel End Clipper 写出的 cell `EndpointIndex`：体部 -1、端帽 1..n），将首分量 **> 0** 的连通片各收成一个面积加权中心，再连到骨架折线上最近的点。若最近点落在边内部，会插入顶点并拆开该边，保证新枝与骨架连通。接到的骨架顶点在加 stub **之前**度数 **> 1**（边内插点后为 2，或接到分叉/干线内点）时，视为开了新分支而不是续上叶端：该 stub 在 cell 数组 **`NewCapBranch`** 上标为 1（其余为 0），并 warning。用于把 MCF 骨架延伸到开口，类似 VMTK 的 Append End Points；默认关闭。
...
| **MinEdgeLength** | `double` | **0.001×包围盒最长边** | **最小边长限制**。局部重网格化过程中，长度短于该值的边缘会被执行折叠操作。自动规则为输入**轴对齐包围盒最长边**的 **0.001** 倍，与 ParaView 内置 `vtkSMBoundsDomain` 的 `scaled_extent`（`scale_factor=0.001`）一致；属性默认 `0` 与手动填 `0` 均表示自动。ParaView 属性面板会在该数值旁显示与 Stream Tracer 等滤镜相同的**缩放**与**按数据重置**控件。仅 **大于 0** 时按该绝对长度使用。 |
...
| **AppendCapEndpoints** | `bool` | false | **接到端帽中心**。开启后，把 cap 数组（默认 cell `EndpointIndex`）首分量 > 0 的每个连通片中心连到骨架上最近的点（必要时拆边）。接到的点度数 > 1 时当作新分支：cell **`NewCapBranch`** = 1 并 warning；接到叶端（度 1）则视为续支，该 stub 为 0。适合已封口网格；数组缺失或没有正值单元时跳过并告警。 |
| **CapEndpointArrayName** | cell array | `EndpointIndex` | **端帽数组**。仅在 `AppendCapEndpoints` 开启时使用；墙体约定为 ≤ 0。 |