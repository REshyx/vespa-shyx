...
顶层 `SmoothingMethod` 默认 0（Shape MCF）；`NumberOfIterations` 默认 1（Fair 忽略该参数）。`DetectFeatureEdges` 默认关（不做 `detect_sharp_edges` / Feature mask 约束，端口 1 为空）。端口 0：平滑后表面；端口 1：特征边诊断（线 + 约束顶点）。
