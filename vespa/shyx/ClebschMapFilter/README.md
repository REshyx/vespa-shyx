...
4. **迭代求解器 (Iterative Solvers)**：
   - 算法采用双层嵌套循环。外层次数由面板 **`OuterLoops`** 控制；内部局部变量 `epsilon`（**不是** SM 属性）从 1 开始，每轮外循环除以 10。
...
| **OuterLoops** | `int` | 5 | 外部松弛循环次数。内部 `epsilon` 随此循环衰减。 |
| **RandomSeed** | `int` | 42 | 初始化波函数时的随机数种子。 |
| **VelocityArrayName** | `string` | 空（回退 `"Velocity"`） | SM 默认空；未设时代码使用 `"Velocity"`。 |
