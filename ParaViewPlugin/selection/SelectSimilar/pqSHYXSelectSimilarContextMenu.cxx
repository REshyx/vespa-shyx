  ...
  pqRepresentation* dataContext, const QStringList&) const
{
  ...
  const bool hasCell =
    pqSHYXGrowSelectionWithSimilarController::HasActiveCellSelection(repr);
  const bool hasPoint =
    pqSHYXGrowSelectionWithSimilarController::HasActivePointSelection(repr);
  if (!hasCell && !hasPoint)
  {
    return false;
  }
  ...
  QAction* selectConnected = new QAction(tr("Select Connected"), menu);
  selectConnected->setObjectName(QStringLiteral("actionSHYXSelectConnected"));
  selectConnected->setToolTip(
    tr("Select every connected point, line, face, or volume in the region(s) "
       "that contain the current selection. Points grow along mesh edges; "
       "lines share a vertex; faces share an edge; volumes share a face. "
       "Disconnected components are left unselected."));
  QObject::connect(selectConnected, &QAction::triggered, menu,
    [reprPtr]() { pqSHYXGrowSelectionWithSimilarController::SelectConnectedRegion(reprPtr); });
  menu->insertAction(insertBefore, selectConnected);
  ...
  if (hasCell)
  {
    ...
    QObject::connect(byNormal, &QAction::triggered, menu,
      [reprPtr]() {
        pqSHYXGrowSelectionWithSimilarController::GrowUntilCompleteByNormal(reprPtr);
      });
