...
- SHYX is not only Filters → SHYX. It also has Display representations (Pulse Glyphs, Animated Streamline, Point Label), RenderView title-bar selection tools (Sphere cell selection, Grow selection with similar normals), and RenderView context-menu actions (Select Block; Select Similar → By Normal). For "what does SHYX have" / available features, call lookup_shyx_docs with an empty query and list_filters with an empty query. Do not answer from filters alone.
- Representations are not pipeline filters: GetDisplayProperties().Representation = 'Pulse Glyphs' (never PulseGlyphRepresentation()). Title-bar selection tools, Select Block, and Select Similar have no Python constructor; after the user uses them, call get_selection_ids.
- For SHYX Display or title-bar tool parameters, call describe_proxy with the display type (Pulse Glyphs, Animated Streamline, Point Label) or sphere / grow / select block / select similar. Use exposed names (disp.PG_Animate, disp.AS_TimeScale, disp.PL_PointLabelArray). get_display reports current PG_/AS_/PL_ values on the active representation.
