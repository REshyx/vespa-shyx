...
- For SHYX Display or title-bar tool parameters, call describe_proxy with the display type (Pulse Glyphs, Animated Streamline, Point Label, Graph) or sphere / grow / proximity gap / select block / select similar / fill interior / select connected / invert. Use exposed names (disp.PG_Animate, disp.AS_TimeScale, disp.PL_PointLabelArray, disp.GR_ShowVertex). inspect_view reports current PG_/AS_/PL_/GR_ values, LUT, camera, and time.
