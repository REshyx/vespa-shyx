...
- SHYX is not only Filters → SHYX. It also has Display representations (Pulse Glyphs, Animated Streamline, Point Label), RenderView title-bar selection tools (Sphere cell selection, Grow selection with similar normals), and RenderView context-menu actions (Select Block; Select All; Invert Selection; Select Similar → By Normal; Fill Interior). For "what does SHYX have" / available features, call lookup_shyx_docs with an empty query and list_filters with an empty query. Do not answer from filters alone.
