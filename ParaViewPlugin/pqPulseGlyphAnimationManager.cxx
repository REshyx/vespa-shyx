...
#include "pqPulseGlyphAnimationManager.h"
...
pqPulseGlyphAnimationManager::pqPulseGlyphAnimationManager(QObject* p)
  : Superclass(p)
{
...
pqPulseGlyphAnimationManager::~pqPulseGlyphAnimationManager() = default;

void pqPulseGlyphAnimationManager::onRenderEnded()
{
  ...
  for (int i = 0; i < reprs.count(); ++i)
  {
    ...
    const int animate =
      repr->GetProperty("PG_Animate") ? vtkSMPropertyHelper(repr, "PG_Animate").GetAsInt() : 0;

    if (representation && (std::string(representation) == "Pulse Glyphs") && visible && animate)
    {
      view->render();
      break;
    }
...
void pqPulseGlyphAnimationManager::onViewAdded(pqView* view)
{
...
void pqPulseGlyphAnimationManager::onViewRemoved(pqView* view)
{
  if (dynamic_cast<pqRenderView*>(view))
  {
