// SHYX adapter replacement for OpenFOAM `src/OpenFOAM/db/error/IOerror.C`.
// globals.C #includes this as IOerror.C (not a Make/files TU). Overlay into lnInclude.
// Same abort-to-throw as foam_error.cxx. SafeFatalIOError also throws when
// FatalIOError.throwing() is set (JobInfo may not be constructed yet).
// OpenFOAM tree stays pristine.
...
)
{
    ...
    else if (FatalIOError.throwing())
    {
        // JobInfo not ready yet, but the host asked for exceptions
        // (snappy run). abort() throws after the adapter exiting() patch.
        FatalIOError
        (
            functionName,
            sourceFileName,
            sourceFileLineNumber,
            ioStream
        )   << msg.c_str() << Foam::abort(FatalIOError);
    }
    else
    {
...
void Foam::IOerror::exiting(const int errNo, const bool isAbort)
{
    if (throwing_)
    {
        // Throw on both exit() and abort() so an embedded host is not killed.
