// SHYX adapter replacement for OpenFOAM `src/OpenFOAM/global/debug/debug.C`.
// globals.C #includes this as debug.C (not a Make/files TU). Overlay into
// lnInclude so the OpenFOAM tree can stay pristine. LoadLibrary: in-memory
// controlDict (no findEtcFiles/home). Missing switch dicts use static
// fallbacks instead of FatalError.
    ...
    if (!controlDictPtr_)
    {
        ...
        shyx_load_log("controlDict: merge in-memory SI defaults");
        {
            ...
            (
                ...
                "        }"
                "        electromagnetic { e e [0 0 1 0 0 1 0] 1.60218e-19; }"
                "        atomic"
                "        {"
                ...
                "    SICoeffs"
                "    {"
                "        kg kg [1 0 0 0 0 0 0] 1;"
                "        m m [0 1 0 0 0 0 0] 1;"
                "        s s [0 0 1 0 0 0 0] 1;"
                "        K K [0 0 0 1 0 0 0] 1;"
                "        mol mol [0 0 0 0 1 0 0] 1;"
                "        A A [0 0 0 0 0 1 0] 1;"
                "        Cd Cd [0 0 0 0 0 0 1] 1;"
                "    }"
                "}\n"
            );
...
)
{
    if (!subDictPtr)
    {
        ...
        if (!subDictPtr)
        {
            // No etc/controlDict on machines without OpenFOAM sources.
            // Static fallbacks: no FatalError / no dictionary::set during ctor.
            static dictionary debugFallback;
            static dictionary infoFallback;
            static dictionary optFallback;
            if (!std::strcmp(subDictName, "DebugSwitches"))
            {
                subDictPtr = &debugFallback;
            }
            else if (!std::strcmp(subDictName, "InfoSwitches"))
            {
                subDictPtr = &infoFallback;
            }
            else
            {
                subDictPtr = &optFallback;
            }
        }
    }
