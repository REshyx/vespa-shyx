// SHYX adapter replacement for OpenFOAM `src/OpenFOAM/db/error/error.C`.
// globals.C #includes this as error.C (not a Make/files TU). Overlay into lnInclude.
// When throwing_ is set, throw on both exit() and abort(). Official OF only throws
// on exit(); abort() calls std::exit(1) on Windows and kills ParaView.
...
void Foam::error::exiting(const int errNo, const bool isAbort)
{
    if (throwing_)
    {
        // Throw on both exit() and abort(). abort() otherwise calls
        // std::exit(1) on Windows and kills an embedded host (ParaView).
