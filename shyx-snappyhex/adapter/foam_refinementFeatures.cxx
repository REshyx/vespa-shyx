...
// SHYX replacement for snappyHexMesh refinementFeatures.C.
// Official read() uses extendedEdgeMesh::New(filename), which constructs a
// dummy Time::New() inside the format reader and abort()s ParaView. Read the
// classified blob with the existing snappy Time via IOobject instead.
...
)
{
    forAll(featDicts, featI)
    {
        ...
        if (!fName.empty())
        {
            // Existing snappy Time. Keeps concave/mixed/normals/regionEdges.
            set(featI, new extendedFeatureEdgeMesh(extFeatObj));

            if (!dryRun_)
            {
                Info<< "Read extendedFeatureEdgeMesh " << extFeatObj.name()
                    << nl << incrIndent;
                operator[](featI).writeStats(Info);
        ...
        else
        {
            ...
            // IFstream: official edgeMesh::New() still uses dummy Time::New().
            IFstream eMeshIs(fName);
            if (!eMeshIs.good())
            {
                FatalIOErrorInFunction(dict)
            ...
            shyx_skip_foamfile_header(eMeshIs);
            pointField eMeshPts;
            edgeList eMeshEds;
            fileFormats::edgeMeshFormat::read(eMeshIs, eMeshPts, eMeshEds);
            const edgeMesh eMesh(eMeshPts, eMeshEds);
