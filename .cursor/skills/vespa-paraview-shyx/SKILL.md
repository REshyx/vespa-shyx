...
- **SHYX 不只是 Filters 菜单**。用户可见能力分几类（清单见 `vespa/INVENTORY.md`；AI 查询约定见 **§9**）：
  ...
  - **状态栏提示**：`ParaViewPlugin/widgets/pqSHYXStatusNotifier*`，状态栏窗口左侧 SHYX chip（不要 `insertPermanentWidget`，那会贴在进度条上）。选择工具的成功/失败计数走这里，不要 `vtkOutputWindow` / Output Messages。
